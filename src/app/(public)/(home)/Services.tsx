...
const Services = () => (
  <section id="services" className="relative flex flex-col items-center justify-center text-center gap-2 py-10 px-8 sm:px-16 md:px-28 lg:px-36 xl:px-40">
    ...
    <div className="grid auto-rows-fr gap-1 cards w-full max-w-screen-lg mx-auto">
      ...
      <Card icon={<DiDocker></DiDocker>} title="Application Development" href={""}>
      ...
      <Card icon={<SiNextdotjs />} title="Fullstack Web Development" href={""}>
      ...
      <Card icon={<FaDiscord></FaDiscord>} title="Discord Development" href={""}>
      ...
      <Card icon={<FaTerminal></FaTerminal>} title="Infrastructure Management" href={""}>
      ...
      <Card icon={<LuFileTerminal></LuFileTerminal>} title="Infrastructure Development & Consultancy" href={""}>
...
const Card = (props: { icon: JSX.Element; title: string; children: any; href: Url }) => (
  <div className="card relative p-8 bg-base-200 border-2 border-base-300 rounded-2xl transition-transform transform hover:scale-105 hover:shadow-md">
    <InView as="div" threshold={.67} initialInView={false} fallbackInView={true}
      onChange={(inView, event) => event.target?.parentElement?.classList.toggle('in-view', inView)}
    ></InView>
    
    <div className="w-full h-full flex flex-col items-start justify-start gap-7 in-view">
